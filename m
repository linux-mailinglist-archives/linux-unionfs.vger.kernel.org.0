...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id F0A1C18789A
	for <lists+linux-unionfs@lfdr.de>; Tue, 17 Mar 2020 05:50:40 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726704AbgCQEuk (ORCPT <rfc822;lists+linux-unionfs@lfdr.de>);
        Tue, 17 Mar 2020 00:50:40 -0400
Received: from sender2-pp-o92.zoho.com.cn ([163.53.93.251]:25336 "EHLO
        sender2-pp-o92.zoho.com.cn" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1726699AbgCQEuk (ORCPT
        <rfc822;linux-unionfs@vger.kernel.org>);
        Tue, 17 Mar 2020 00:50:40 -0400
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed; t=1584420593;
        s=zohomail; d=mykernel.net; i=cgxu519@mykernel.net;
        h=Date:From:Reply-To:To:Cc:Message-ID:In-Reply-To:References:Subject:MIME-Version:Content-Type:Content-Transfer-Encoding;
        bh=iFnesi+CEBAcCsIfN76jr+5yv+Ksd7sOFlzG3BwaJ0s=;
        b=Jv17NYwN5Ra6hnVb+Z7gAoFXVhvNGqK6Bw+OedGCcIqP0giNVuB3YFwCC4sHPdFw
        kr8Jpms6d3zG5O4Fn5UI2+RM8qWvGQ/7JFksOyMEyqmyRjxDs3hG19WKBbeYOvDGREi
        E3ymkYYHHFKGSwhCH9QBGfZOS0XO0OkamHivD7qM=
Received: from mail.baihui.com by mx.zoho.com.cn
        with SMTP id 1584420591458588.3630113244926; Tue, 17 Mar 2020 12:49:51 +0800 (CST)
Date:   Tue, 17 Mar 2020 12:49:51 +0800
...
Message-ID: <170e6d3975b.f31c4fdf10742.467886206992391518@mykernel.net>
In-Reply-To: <170e6cbc06e.d92f059210735.6409036322143087221@mykernel.net>
References: <20200210031009.61086-1-cgxu519@mykernel.net> <170e6cbc06e.d92f059210735.6409036322143087221@mykernel.net>
...
X-Mailing-List: linux-unionfs@vger.kernel.org
 ...
 ---- =E5=9C=A8 =E6=98=9F=E6=9C=9F=E4=BA=8C, 2020-03-17 12:41:17 Chengguang=
 Xu <cgxu519@mykernel.net> =E6=92=B0=E5=86=99 ----
 >  ---- =E5=9C=A8 =E6=98=9F=E6=9C=9F=E4=B8=80, 2020-02-10 11:10:09 Chenggu=
ang Xu <cgxu519@mykernel.net> =E6=92=B0=E5=86=99 ----
 ...
 >  >=20
 >  > (1) Performance
 >  > Synchronization is probably heavy because it actually syncs unnecessa=
ry
 ...
 >  > reduce cost of synchronization and will not seriously impact IO perfo=
rmance
 >  > of unrelated processes. In special case, when having very few dirty i=
nodes
 >  > and a lot of clean upper inodes in overlayfs, then iteration may wast=
e
 ...
 > I tested xfstest overlay part (and generic/427 which is testcase for syn=
cfs) and heavy untar + syncfs test and I found no problem.
 >=20
...
cgxu
 ...
 >  > Changes since v10:
 >  > - Add special handling in ovl_evict_inode() for inode eviction triger=
ed
...
++
 ...
 >  > =20
 >  >  overlay-objs :=3D super.o namei.o util.o inode.o file.o dir.o readdi=
r.o \
 >  > -        copy_up.o export.o
...
,
 ...
 >  >  {
 >  > @@ -466,3 +471,7 @@ int ovl_set_origin(struct dentry *dentry, struct =
dentry *lower,
 >  > =20
 >  >  /* export.c */
 ...
 >  > @@ -183,6 +183,7 @@ static struct inode *ovl_alloc_inode(struct super=
_block *sb)
 >  >      oi->lower =3D NULL;
 ...
 >  > -
 >  > -    /*
 >  > -     * If this is a sync(2) call or an emergency sync, all the super=
 blocks
 >  > -     * will be iterated, including upper_sb, so no need to do anythi=
ng.
 >  > -     *
...
ing
 ...
 >  > @@ -376,11 +347,19 @@ static int ovl_remount(struct super_block *sb, =
int *flags, char *data)
 >  >      return 0;
 >  >  }
 >  > =20
 ...
 >  > @@ -1601,6 +1580,9 @@ static int ovl_fill_super(struct super_block *s=
b, void *data, int silent)
 ...
 >  > +
 >  > +/**
 >  > + * upper_inodes list is used for organizing potential target of sync=
fs,
 ...
 >  > + * evicting.
 >  > + *
 >  > + * Introduce ovl_inode flag "OVL_EVICT_PENDING" to indicate the ovl =
inode
 >  > + * is in eviction process, syncfs(actually ovl_sync_inodes()) will w=
ait on
 >  > + * evicting inode until the IO to be completed in evict_inode().
 >  > + *
 >  > + * inode state/ovl_inode flags cycle in syncfs context will be as be=
low.
 >  > + * OVL_EVICT_PENDING (ovl_inode->flags) is only marked when inode ha=
ving
 >  > + * upper inode.
 >  > + *
 >  > + * (allocation)
 ...
 >  > + *
 >  > + * upper_sb->s_umount(semaphore)    : avoiding r/o to r/w or vice ve=
rsa
 >  > + * sb->s_sync_lock(mutex)           : avoiding concorrent syncfs run=
ning
 >  > + * ofs->upper_inodes_lock(spinlock) : protecting upper_inodes list
 >  > + * sb->s_inode_wblist_lock(spinlock): protecting s_inodes_wb(sync wa=
iting) list
 ...
 >  > + * @sb: The overlayfs super block
 >  > + *
 >  > + * upper_inodes list is used for organizing ovl inode which has uppe=
r inode,
 >  > + * by iterating the list to looking for and syncing dirty upper inod=
e.
 >  > + *
 >  > + * When starting syncing inode, we add the inode to wait list explic=
itly,
 >  > + * in order to save memory we reuse inode->i_wb_list and sb->s_inode=
s_wb,
 ...
 >  > +
 >  > +    /*
 >  > +     * ovl inode in sb->s_inodes_wb list has already got inode refer=
ence,
 >  > +     * this will avoid silent inode droping during waiting on it.
 >  > +     *
 >  > +     * Splice the sync wait list onto a temporary list to avoid wait=
ing on
...
ing
 ...
 >  > @@ -386,13 +386,33 @@ void ovl_dentry_set_redirect(struct dentry *den=
try, const char *redirect)
 ...
 >  >      if (lowerdata)
 >  > @@ -421,6 +441,7 @@ void ovl_inode_update(struct inode *inode, struct=
 dentry *upperdentry)
 ...
 >  >  }
 >  > =20
 >  >  static void ovl_dentry_version_inc(struct dentry *dentry, bool impur=
ity)
 >  > --=20
 >  > 2.21.1
 >  >=20
